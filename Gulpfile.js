...
gulp.task("style-in", function() {
  gulp
    ...
    .pipe(
      sass({ outputStyle: "expanded", lineNumbers: true }).on(
        "error",
        sass.logError
      )
    )
...
gulp.task("img-in", function() {
  gulp.src(path.IMAGES).pipe(gulp.dest("./builds/inbound/img"));
});
...
gulp.task("fonts-in", function() {
  gulp.src(path.FONTS).pipe(gulp.dest("./builds/inbound/fonts"));
});

gulp.task("fonts-out", function() {
  gulp.src(path.FONTS).pipe(gulp.dest("./builds/outbound/fonts"));
});

// OPERATIONAL TASKS ---------------------------------------------------->>>>>>>>

gulp.task("clean-in", function() {
  return gulp.src(path.INBOUND, { read: false }).pipe(clean());
});

gulp.task("clean-out", function() {
  return gulp.src(path.OUTBOUND, { read: false }).pipe(clean());
});
